...
function convert({ headers, frames, resources, index, id }, { DOMParser, enableScripts, fetchMissingResources } = { DOMParser: globalThis.DOMParser }) {
    ...
    while (nodes.length) {
        ...
        for (const child of childNode.childNodes) {
            ...
            if (child.tagName === LINK_TAG && href) {
                ...
                } else if (child.tagName === SCRIPT_TAG && !fetchMissingResources) {
            ...
            } else if (child.tagName === META_TAG && !fetchMissingResources) {
    ...
    if (fetchMissingResources) {
        ...
        const metaCSP = document.createElement(META_TAG);
