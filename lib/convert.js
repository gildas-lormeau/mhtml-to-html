                ...
                if (tagName === LINK_TAG && href) {
                    ...
                    if (rel) {
                        ...
                        if (rel === REL_ATTRIBUTE_STYLESHEET) {
                            ...
                            if (!fetchMissingResources) {
                                const linkTitle = child.getAttribute(TITLE_ATTRIBUTE);
                                if (linkTitle && rel.includes(REL_ATTRIBUTE_ALTERNATE)) {
                    ...
                    if (src) {
                        ...
                        if (resource) {
                            ...
                            if (tagName === EMBED_TAG || tagName === OBJECT_TAG || isMedia(resource.contentType)) {
                                if (!fetchMissingResources) {
                                    resource.used = true;
...
function createDocument(resources) {
    for (const resource of Object.values(resources)) {
        ...
        if (isImage(contentType)) {
            ...
            return { ...resource, contentType: TEXT_HTML_CONTENT_TYPE, transferEncoding: undefined,
                data: `<html><body><pre>${escapeText(String(resource.data))}</pre></body></html>` };
        }
    }
}
    ...
    if (ast) {
        cssTree.walk(ast, node => {
            if (node.type === URL_FUNCTION) {
                ...
                const importNode = node.prelude.children.first;
                const path = importNode.value;
                if (!path.startsWith(DATA_PROTOCOL) && !path.startsWith(ORIGINAL_URL_FUNCTION_NAME)) {
                    const id = resolvePath(path, base);
                    const resource = getResource(resources, id, path);
                    if (resource) {
