                            ...
                            if (child.tagName && child.tagName.toUpperCase() === EMBED_TAG || child.tagName && child.tagName.toUpperCase() === OBJECT_TAG) {
                                ...
                                if (fetchMissingResources) {
                                    ...
                                    if (child.tagName && child.tagName.toUpperCase() === IFRAME_TAG) {
                                        ...
                                        setAttribute(child, attribute, DATA_PROTOCOL + TEXT_HTML_CONTENT_TYPE + DATA_URI_PAYLOAD_SEPARATOR + encodeURIComponent(result));
                                    }
                                }
                            }
                        } else if (fetchMissingResources) {
            ...
            if (node.type === URL_FUNCTION) {
                ...
                if (!path.startsWith(DATA_PROTOCOL) && !path.startsWith(ORIGINAL_URL_FUNCTION_NAME)) {
                    ...
                    if (resource) {
                        ...
                        if (!missingResources) {
                            resource.used = true;
                            node.prelude.children.first.value = getCSSFunction(id) + getResourceURI(resource);
                        }
                    } else if (missingResources) {
                        addMissingResource(missingResources, id);
                    } else {
                        unfoundResources.add(id);
                        node.prelude.children.first.value = getCSSFunction(id) + id;
                    }
                }
            }
        });
...
function getCSSFunction(id) {
    return ORIGINAL_URL_FUNCTION_NAME + CSS_FUNCTION_PARENTHESIS_START + JSON.stringify(id) + CSS_FUNCTION_PARENTHESIS_END;
}
