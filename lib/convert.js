...
function convert({ headers, frames, resources, index, id }, { DOMParser, enableScripts, fetchMissingResources } = { DOMParser: globalThis.DOMParser }) {
    ...
    if (!enableScripts) {
        document.querySelectorAll("script:not([type=\"application/ld+json\"])").forEach(scriptElement => scriptElement.remove());
    }
    const baseElement = document.querySelector(BASE_TAG);
    if (baseElement) {
    ...
    if (!fetchMissingResources) {
        ...
        document.querySelectorAll("meta[" + HTTP_EQUIV_ATTRIBUTE + "=content-security-policy]").forEach(element => element.remove());
        const metaCSP = document.createElement("meta");
        metaCSP.setAttribute(HTTP_EQUIV_ATTRIBUTE, "content-security-policy");
        ...
        document.querySelectorAll("meta[" + HTTP_EQUIV_ATTRIBUTE + "=refresh]").forEach(element => element.remove());
        document.querySelectorAll("[" + INTEGRITY_ATTRIBUTE + "]").forEach(element => element.removeAttribute(INTEGRITY_ATTRIBUTE));
        resource.used = true;
    }
    const stylesheets = {};
    ...
    while (nodes.length) {
        ...
        for (const child of childNode.childNodes) {
            ...
            if (child.tagName === LINK_TAG && href) {
            ...
            } else if (child.tagName === STYLE_TAG) {
            ...
            } else if (child.tagName === IMG_TAG || child.tagName === AUDIO_TAG || child.tagName === VIDEO_TAG || child.tagName === SOURCE_TAG || child.tagName === SCRIPT_TAG) {
                if (src) {
                ...
                if (child.tagName === IMG_TAG || child.tagName === SOURCE_TAG) {
            ...
            } else if (child.tagName === BODY_TAG || child.tagName === TABLE_TAG || child.tagName === TD_TAG || child.tagName === TH_TAG) {
            ...
            } else if (child.tagName === INPUT_TAG) {
            ...
            } else if (child.tagName === IFRAME_TAG || child.tagName === FRAME_TAG || child.tagName === EMBED_TAG || child.tagName === OBJECT_TAG) {
                let id, attribute;
                ...
                if (src) {
                    ...
                    if (resource) {
                        if (child.tagName === EMBED_TAG || child.tagName === OBJECT_TAG) {
                            if (!fetchMissingResources) {
                                resource.used = true;
            ...
            } else if ((child.tagName === A_TAG || child.tagName === AREA_TAG) && !fetchMissingResources) {
                if (href) {
                    try {
            ...
            if (node.type === "Url") {
                ...
                if (!path.startsWith(DATA_PROTOCOL)) {
                    ...
                    if (resource) {
                        if (!missingResources) {
                            ...
                            node.value = "--mhtml-to-html-url(" + JSON.stringify(id) + ") " + getResourceURI(resource);
                        }
                    } else if (missingResources) {
                        addMissingResource(missingResources, id, BASE64_ENCODING);
                    } else {
                        node.value = "--mhtml-to-html-url(" + JSON.stringify(id) + ") " + id;
                    }
                }
                ...
                if (!path.startsWith(DATA_PROTOCOL)) {
                    ...
                    if (resource) {
                        ...
                        if (!missingResources) {
                            resource.used = true;
                            node.prelude.children.first.value = "--mhtml-to-html-url(" + JSON.stringify(id) + ") " + getResourceURI(resource);
                        }
                    } else if (missingResources) {
                        addMissingResource(missingResources, id);
                    } else {
                        node.prelude.children.first.value = "--mhtml-to-html-url(" + JSON.stringify(id) + ") " + id;
                    }
                }
            }
        });
