    ...
    if (config.fetchMissingResources) {
        ...
        if (missingResources.length) {
            await Promise.all(missingResources.map(async resource => {
                ...
                let url = id;
                const urnErrorMatch = url.match(/^urn:[^:]+:(.+)$/);
                if (urnErrorMatch) {
                    url = urnErrorMatch[1];
                }
                try {
...
function convert({ headers, frames, resources, index, id }, { DOMParser, enableScripts, fetchMissingResources } = { DOMParser: globalThis.DOMParser }) {
    ...
    const unfoundResources = new Set();
    while (nodes.length) {
        const childNode = nodes.shift();
        if (childNode.childNodes) {
            for (const child of childNode.childNodes) {
                ...
                if (child.getAttribute) {
                    ...
                    if (style) {
                        const declarations = replaceStyleSheetUrls(resources, base, { data: style }, { context: "declarationList" }, stylesheets, fetchMissingResources && missingResources, unfoundResources);
                        if (!fetchMissingResources) {
                ...
                if (child.tagName && child.tagName.toUpperCase() === LINK_TAG && href) {
                    ...
                    const shadowModeAttribute = child.getAttribute("shadowmode");
                    if (shadowModeAttribute) {
                        child.removeAttribute("shadowmode");
                        child.setAttribute("shadowrootmode", shadowModeAttribute);
                    }
                }
                nodes.push(child);
            }
...
function addMissingResource(missingResources, id, transferEncoding) {
    if ((id.startsWith("http:") || id.startsWith("https:") || id.startsWith("urn:")) && !missingResources.find(resource => resource.id === id)) {
