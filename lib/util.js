...
async function decodeMimeHeader(encodedSubject) {
    if (encodedSubject && encodedSubject.startsWith("=?") && encodedSubject.endsWith("?=")) {
        return await decodeNextPart();
    } else {
        return encodedSubject || "";
    }

    async function decodeNextPart(encodedSubjectParts = [], index = 0) {
        ...
        if (encoding === "Q") {
            ...
            const dataURI = `data:text/plain;charset=${charset};base64,${value}`;
            const response = await fetch(dataURI);
            encodedSubjectParts.push(await response.text());
        }
        if (endValue + 2 < encodedSubject.length) {
            return decodeNextPart(encodedSubjectParts, endValue + 2);
        } else {
            return encodedSubjectParts.join("");
        }
    }
}
