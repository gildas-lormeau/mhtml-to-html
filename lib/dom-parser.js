...
const treeAdapter = {
    ...
    createDocument() {
        ...
        Object.defineProperty(document,
            DOCUMENT_ELEMENT_PROPERTY_NAME,
            {
                get() {
                    return this.childNodes.find(node => testElement(node,
                        TAG_NAMES.HTML));
                }
            });
        Object.defineProperty(document,
            ...
            {
                get() {
                    return this.documentElement.childNodes.find(node => testElement(node,
                        TAG_NAMES.HEAD));
                }
            });
        return document;
    ...
    createElement(tagName,
        namespaceURI,
        attrs) {
        const element = defaultTreeAdapter.createElement(tagName,
            namespaceURI,
            attrs);
...
function setAttribute(name,
    value) {
    const indexAttribute = findIndexAttribute(this,
        name);
    if (indexAttribute === -1) {
        this.attrs.push({
            name,
...
function getAttribute(name) {
    const attribute = findAttribute(this,
        name);
    if (attribute) {
...
function removeAttribute(name) {
    const indexAttribute = findIndexAttribute(this,
        name);
    if (indexAttribute !== -1) {
        this.attrs.splice(indexAttribute,
            1);
    }
}
...
function findAttribute(element,
    name) {
    return element.attrs.find(attr => testAttributeName(attr,
        name));
}

function findIndexAttribute(element,
    name) {
    return element.attrs.findIndex(attr => testAttributeName(attr,
        name));
}

function testAttributeName(attr,
    name) {
    return testCaseInsensitive(attr.name,
        name);
}

function appendChild(child) {
    return treeAdapter.appendChild(this,
        child);
}
...
function remove() {
    if (this.parentNode) {
        ...
        if (indexNode !== -1) {
            this.parentNode.childNodes.splice(indexNode,
                1);
...
function replaceWith(...nodes) {
    if (this.parentNode) {
        const indexNode = findIndexNode(this);
        if (indexNode !== -1) {
            const oldNodes = this.parentNode.childNodes.splice(indexNode,
                1,
                ...nodes);
...
function testElement(node,
    name) {
    return node.tagName && testCaseInsensitive(node.tagName,
        name);
}

function testCaseInsensitive(stringLeft,
    stringRight) {
...
function serializeTextNode(textNode) {
    if (shouldNotEscapeEntities(textNode)) {
        if (shouldEscapeTextNode(textNode)) {
...
function shouldNotEscapeEntities(textNode) {
...
function escapeTextNode(textNode) {
    return textNode.value
        .replace(CLOSING_TAG_MARKER_REGEXP,
            ESCAPED_CLOSING_TAG_MARKER)
        .replace(SELF_CLOSING_TAG_MARKER_REGEXP,
            ESCAPED_SELF_CLOSING_TAG_MARKER);
}
...
function escapeEntitiesTextNode(textNode) {
    return textNode.value
        .replace(AMPERSAND_REGEXP,
            AMPERSAND_ENTITY)
        .replace(NON_BREAKING_SPACE_REGEXP,
...
function serializeAttributes(element) {
    return joinArray(element.attrs.map(({ name,
        value,
        prefix,
        namespace }) => {
        if (!name.match(INVALID_TAG_NAME_REGEXP)) {
            ...
            if (namespace === NS.XLINK) {
                return serializeAttribute(name,
                    value,
                    XLINK_PREFIX);
...
function serializeAttribute(name,
    value,
    prefix) {
    if (prefix) {
