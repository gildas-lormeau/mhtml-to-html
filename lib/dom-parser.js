...
const treeAdapter = {
    ...
    createDocument() {
        ...
        Object.defineProperty(document, DOCUMENT_ELEMENT_PROPERTY_NAME, {
            get() {
                return document.childNodes.find(node => node.tagName && node.tagName.toLowerCase() === TAG_NAMES.HTML);
            }
        });
        Object.defineProperty(document, HEAD_PROPERTY_NAME, {
            get() {
                return document.documentElement.childNodes.find(node => node.tagName && node.tagName.toLowerCase() === TAG_NAMES.HEAD);
            }
        });
        return document;
    },
...
function serializeTextNode(textNode) {
    ...
    if (!parentTagName || TEXT_NODE_TAG_NAMES.includes(parentTagName)) {
        if ((parentTagName === TAG_NAMES.SCRIPT && (!parentType || parentType === JAVASCRIPT_MIME_TYPE)) || parentTagName === TAG_NAMES.STYLE) {
...
function serializeElement(element) {
    ...
    let html = OPENING_TAG_MARKER + tagName.toLowerCase();
