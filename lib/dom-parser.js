...
function serializeTextNode(textNode) {
    ...
    if (!parentTagName || TEXT_NODE_TAG_NAMES.includes(parentTagName.toLowerCase())) {
        ...
        return textNode.value
            ...
            .replace(/\u00a0/g, NON_BREAKING_SPACE_ENTITY)
    ...
    return element.attrs.map(({ name, value, prefix, namespace }) => {
        if (!name.match(/["'>/=]/)) {
            ...
            if (namespace) {
                ...
                } else if (namespace === NS.MATHML) {
                    return ` math:${name}="${value}"`;
                } else if (namespace === NS.SVG) {
                    return ` svg:${name}="${value}"`;
                } else {
                    if (prefix === "") {
                        return ` ${name}="${value}"`;
                    } else {
