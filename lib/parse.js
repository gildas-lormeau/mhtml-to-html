...
function getMetaCharsetElement(document) {
    ...
    let metaElement;
    while (nodes.length && !metaElement) {
        const childNode = nodes.shift();
        if (childNode.childNodes) {
            for (let childIndex = 0; childIndex < childNode.childNodes.length && !metaElement; childIndex++) {
                const child = childNode.childNodes[childIndex];
                if (child.tagName && child.tagName.toUpperCase() === META_TAG && child.getAttribute(CHARSET_ATTRIBUTE)) {
                    return child;
                }
                nodes.push(child);
            }
...
function getMetaContentTypeElement(document) {
    ...
    while (nodes.length && !metaElement) {
        ...
        if (childNode.childNodes) {
            for (let childIndex = 0; childIndex < childNode.childNodes.length && !metaElement; childIndex++) {
                const child = childNode.childNodes[childIndex];
                if (child.tagName && child.tagName.toUpperCase() === META_TAG && child.getAttribute(HTTP_EQUIV_ATTRIBUTE) === CONTENT_TYPE_HEADER) {
                    return child;
                }
                nodes.push(child);
            }
