    ...
    while (nodes.length) {
        ...
        for (const child of childNode.childNodes) {
            ...
            if (child.tagName == "LINK") {
                ...
                let rel = child.getAttribute("rel");
                if (rel) {
                    rel = rel.toLowerCase();
                }
                if (resource && rel === "stylesheet") {
                    ...
                    resource.data = replaceStyleSheetUrls(resources, base, resource, { context: "stylesheet" }, stylesheets);
                    styleElement.appendChild(document.createTextNode(resource.data));
                    childNode.replaceChild(styleElement, child);
                }
                if (resource && rel && rel.includes("icon")) {
                    try {
                ...
                resource = getResource(resources, src, child.getAttribute(SRC_ATTRIBUTE));
                if (resource && (
                    (child.parentNode.tagName === "AUDIO") ||
                    (child.parentNode.tagName === "VIDEO") ||
                    (child.parentNode.tagName === "PICTURE"))) {
                    try {
                        child.setAttribute(SRC_ATTRIBUTE, getResourceURI(resource));
                    } catch (_) {
                        // ignored
            ...
            } else if (child.tagName == "INPUT" && child.getAttribute("type") === "image") {
                resource = getResource(resources, src, child.getAttribute(SRC_ATTRIBUTE));
                if (resource) {
                    try {
                        child.setAttribute(SRC_ATTRIBUTE, getResourceURI(resource));
                ...
                if (src) {
                    ...
                    if (src.startsWith(CID_PROTOCOL)) {
                        ...
                        id = src;
                        frame = getResource(resources, src, child.getAttribute(SRC_ATTRIBUTE));
                    }
                    if (frame) {
                        const html = convert({
    ...
    if (ast) {
        cssTree.walk(ast, node => {
            if (node.type === "Url") {
                ...
                const id = resolvePath(removeQuotes(path), base);
