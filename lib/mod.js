...
async function convert({ headers, frames, resources, index }, { DOMParser, enableScripts } = { DOMParser: globalThis.DOMParser, enableScripts: false }) {
    ...
    if (headers) {
        const pageInfo = {
            ...
            "@type": "WebPage",
            "url": index,
            "name": await decodeMimeHeader(headers["Subject"]),
            "dateCreated": headers["Date"],
            "additionalProperty": {
                "@type": "PropertyValue",
                "name": "savedBy",
                "value": await decodeMimeHeader(headers["From"])
            }
        };
        ...
        if (document.head.firstChild) {
            document.head.insertBefore(pageInfoElement, document.head.firstChild);
        } else {
    ...
    while (nodes.length) {
        ...
        for (const child of childNode.childNodes) {
            ...
            if (child.tagName == "LINK") {
                ...
                if (child.getAttribute(BACKGROUND_ATTRIBUTE)) {
                    resource = resources[child.getAttribute(BACKGROUND_ATTRIBUTE)];
                    if (resource && isImage(resource.contentType)) {
                        try {
...
function getResourceURI({ contentType, transferEncoding, data }) {
    return `data:${contentType};${BASE64_ENCODING},${transferEncoding === BASE64_ENCODING ? data : encodeStringToBase64(data)}`;
}
