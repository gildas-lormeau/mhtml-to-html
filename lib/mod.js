...
function parse(mhtml, { DOMParser } = { DOMParser: globalThis.DOMParser }, context = { resources: {}, frames: {} }) {
    ...
    function parseResourceData() {
        ...
        while (!boundaryFound && indexMhtml < mhtml.length - 1) {
            ...
            if (indexBoundary !== -1) {
                ...
                if (indexBoundary > 1 && endsWithCRLF(next)) {
                    next = next.slice(0, indexBoundary - 2);
                } else if (indexBoundary > 0 && endsWithLF(next)) {
                    next = next.slice(0, indexBoundary - 1);
                } if (indexBoundary === 0) {
                    next = [];
                }
                boundaryFound = true;
    ...
    function processStyleSheetCharset(charset) {
        try {
            ...
            if (ast.children.first && ast.children.first.type === AT_RULE && ast.children.first.name === "charset") {
    ...
    function processDocumentCharset(charset) {
        ...
        try {
            if (charserMetaElement) {
                let htmlCharset = charserMetaElement.getAttribute("charset");
                if (htmlCharset) {
